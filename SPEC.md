...
字体路径：正文使用“拉丁字体 + 中文字体”双字体绘制，按 Unicode 标量拆分绘制/测量片段（拉丁、CJK、全角与扩展 B 代理对使用中文字体，其余使用拉丁字体），行高取两者较大值。默认预设 Consolas + 等线，菜单可切换 Consolas + 黑体、等线、黑体；字体状态仅保存在当前实例，不写配置。

颜色渲染路径：默认关闭。开启时仅对 `.ini`/`.env` 文档逐行做无分配标记（注释、节名、键、操作符、字符串、数字），每种标记按浅色/深色主题映射前景色；选中文字保留标记前景色、仅换背景色。标记逻辑位于 `editor/ini_env_highlight` 纯函数模块，由单元测试覆盖，不缓存正文。
...
- `editor`：行偏移索引、选择模型、Unicode 字形宽度命中、Tab 显示/插入策略、输入导航、系统色 GDI 自绘、双字体（拉丁/中文）绘制和 `.ini`/`.env` 语法着色；
- `app`：原生菜单、系统主题跟随、字体/颜色渲染开关、安全状态、关于窗口、Explorer 文件拖放、保存提示及锁屏/电源/会话策略。
